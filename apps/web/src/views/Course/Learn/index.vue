...
const isWordBlurred = ref(false); //是否模糊显示单词 默认是模糊的
...
const onInput = (index: number) => {
    ...
    nextTick(() => {
        const inputs = inputRefs.value as HTMLInputElement[] | null;
        if (inputs && inputs.length > 0 && index < inputs.length - 1) { //不是最后一个输入框
            const nextInput = inputs[index + 1];
            if (nextInput) {
                nextInput.focus(); //下一个输入框聚焦
            }
        }
    })
}
...
const onKeyDown = (index: number, event: KeyboardEvent) => {
    ...
    if (event.key === 'Backspace') {
        ...
        nextTick(() => {
            const inputs = inputRefs.value as HTMLInputElement[] | null;
            if (inputs && inputs.length > 0 && index > 0) { //不是第一个输入框
    ...
    if (event.key.length == 1 && !event.ctrlKey && !event.metaKey && !event.altKey) {
        if (current.input && index < wordList.value.length - 1) {
            event.preventDefault();
            const nextWord = wordList.value[index + 1];
            if (!nextWord) return;
