...
const getInstance = (options: Options): SpeechRecognition => {
    ...
    if (!instance) {
        ...
        instance.lang = lang // 语言
        instance.continuous = continuous // 是否连续识别
        instance.interimResults = interimResults // 是否显示临时结果
        instance.maxAlternatives = maxAlternatives // 最大候选数
    }
    //其他次数就直接返回了
    return instance
}
