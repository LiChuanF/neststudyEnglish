...
import { createDeepSeek } from '../llm'
    ...
    async handleEmailDigest() {
                ...
                wordBookRecords: {
                    ...
                    some: {
                        createdAt: {
                            ...
                            lt: dayjs().add(1, 'day').startOf('day').toDate() //<明天00:00:00
                        }
                    }
                }
            },
        ...
        for (const user of userIds) {
            const agent = createAgent({
                model: createDeepSeek(), // 深seek模型
                tools: [this.queryTool()], // 工具 连接数据库
            ...
            const content = result.messages.at(-1)?.content // 最后一条消息是大模型的回复
            if (content) {
